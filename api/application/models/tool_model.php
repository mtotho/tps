...
class Tool_model extends CI_Model{
	...
	function getTools(){
		...
		$categories = array();
		for($i=1; $i<4; $i++){
			$fundamentals = $this->fundamental_model->getByCategoryId($i);

			//$categories["".$i.""]["fundamentals"]=$this->fundamental_model->getFundamentals();

			$k=0;
			foreach($fundamentals as $f){

				$fundamentals[$k]['tools']=$this->getByFundamentalId($f['id']);
				$k++;
			}

			$categories["".$i.""]["fundamentals"]=$fundamentals;
		}
